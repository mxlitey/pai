...
- 🗓️ **月视图**：7 列周网格（含相邻月补位，5–6 行），今日高亮，每格最多 3 条，超出显示「+N 更多」
...
- 🗂️ **排课管理**：学员 / 日期范围 / 课程组合筛选，新增（单条或批量，日期×学员笛卡尔积）、编辑（含跨月跨学员迁移）、删除
- ✅ **点名管理**：按日期点名、课程×时段平铺分块、三态标记（到课 / 未点名 / 缺勤）、一键全部到课
- 📢 **公告管理**：编辑 / 预览双 tab，Markdown 实时渲染，字数统计
- 🔗 **分享链接**：一键生成全部学员查看链接，单条 / 全量复制
- 📊 **看板数据**：按自然月 / 日期范围查看统计卡（排课记录 / 天数 / 学员 / 班型）、每日安排与考勤矩阵（班型图例可点击高亮）
...
- 🛡️ **防竞态**：模块级写锁串行化读-改-写，多锁按字典序加锁防死锁（仅在单个边缘函数实例内有效）
...
2. 在首页搜索框输入学员姓名，选中学员后点击「查看排课」进入日历页并加载排课
...
| POST | `/api/schedule-add-batch` | 是 | 批量新增排课（日期×学员笛卡尔积，单条即 1 日期 × 1 学员）；新增成功的记录在 `createdSchedules` 返回（含 id），重复项计入 `skipped` 并在 `errors[].existing` 回传已存在记录 |
...
| `defaultStartTime` | string | 是 | 默认开始时间，格式 `HH:mm`（新增/更新课程时必填；MCP 批量排课工具缺省时间时自动套用） |
| `defaultEndTime` | string | 是 | 默认结束时间，格式 `HH:mm`（新增/更新课程时必填；MCP 批量排课工具缺省时间时自动套用） |
