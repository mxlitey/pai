// SQLite 存储层 —— 替代 EdgeOne Pages Blob
// 保持与原 store.js 完全相同的导出函数签名，业务接口零改动
// 数据组织（与原 Blob 版等价，但用 SQLite 表存储）：
//   students   表 -> 原 students/index.json
//   courses    表 -> 原 courses/index.json
//   schedules  表 -> 原 schedules/{studentId}/{yyyy-MM}.json（取消按月分文件，改用索引）
//   announcement 表 -> 原 config/announcement.json（单行）
//   admin      表 -> 超管账号（为后期多账号体系预留）
//   settings   表 -> 通用键值配置（为 VITE_APP_NAME 后台设置等预留）
...
// ========== 超管账号（为后期多账号体系预留） ==========
// 当前阶段：单超管，密码来自 ADMIN_PASSWORD 或引导设置
